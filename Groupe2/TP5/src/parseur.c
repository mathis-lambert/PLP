...
// Fonction pour parser les expressions lambda
int parse_lambda_expression(Token *tokens, int token_count, LambdaExpression *lambda)
{
    if (tokens[0].type == TOKEN_LAMBDA && tokens[1].type == TOKEN_IDENTIFIER && tokens[2].value[0] == '.')
    {
        // Stocker la variable
        strcpy(lambda->variable_name, tokens[1].value);

        // Stocker l'expression (tous les tokens après le point)
        lambda->expression_tokens = &tokens[3];
        lambda->expression_token_count = token_count - 3;
        return 1; // Succès
    }
    return 0; // Erreur
}
