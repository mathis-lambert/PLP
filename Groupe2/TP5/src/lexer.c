...
// Fonction pour extraire une variable
Token get_identifier(const char** input) {
    Token token;
...
int tokenize(const char* input, Token* output) {
    ...
    while (*current != '\0') {
        ...
        if (isdigit(*current) || *current == '.') {
            ...
            // Tokeniser un identifiant ou le mot clé "lambda"
            output[(token_count)++] = get_identifier(&current);
        } else if (*current == '\0') {
